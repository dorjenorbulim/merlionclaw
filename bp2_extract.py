#!/usr/bin/env python3
"""
BP2 Content Extractor
Extracts text from PDF, DOC, DOCX files in the BP2 directory structure.
"""
...
def extract_doc(filepath):
    ...
    # Try to read as plain text first (some .doc files are actually text)
    try:
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
            # Check if it looks like binary (has many null bytes or control chars)
            if '\x00' in content[:1000]:
                return None  # Binary file, need proper converter
            return content
    except:
        return None
...
def process_directory():
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write(f"BP2 Content Extraction Summary\n")
